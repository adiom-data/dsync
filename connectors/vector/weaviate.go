...
func NewWeaviateConnector(host string, scheme string, groupID string, dataMapper func(interface{}) interface{}) *weaviateConnector {
	cfg := weaviate.Config{
		Host:   host,
		Scheme: scheme,
	}
...
func (c *weaviateConnector) UpsertDocuments(ctx context.Context, namespace string, docs []*VectorDocument) error {
	...
	slog.Info("success", "count", delResp.Results.Successful)
