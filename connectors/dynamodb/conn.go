...
func (c *conn) GeneratePlan(ctx context.Context, r *connect.Request[adiomv1.GeneratePlanRequest]) (*connect.Response[adiomv1.GeneratePlanResponse], error) {
	...
	for _, name := range tableNames {
		eg.Go(func() error {
			...
			if r.Msg.GetParallelize() && c.docsPerSegment > 0 {
				totalSegments = int(tableDetails.Count / uint64(c.docsPerSegment))
...
func (c *conn) StreamUpdates(ctx context.Context, r *connect.Request[adiomv1.StreamUpdatesRequest], s *connect.ServerStream[adiomv1.StreamUpdatesResponse]) error {
	...
	// TODO have own stream client
	streamMult := NewStreamMult(c.client.streamsClient, state, ch)
...
func NewConn() adiomv1connect.ConnectorServiceHandler {
	// TODO: factor out
	client, err := NewClient(context.Background(), "http://localhost:4566")
	if err != nil {
		panic(err)
	}
	return &conn{
