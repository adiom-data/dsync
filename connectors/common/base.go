...
func (c *connector) IntegrityCheck(ctx context.Context, task iface.IntegrityCheckQuery) (iface.ConnectorDataIntegrityCheckResult, error) {
	if task.CountOnly {
		res, err := c.impl.GetNamespaceMetadata(ctx, connect.NewRequest(&adiomv1.GetNamespaceMetadataRequest{Namespace: &adiomv1.Namespace{Db: task.Db, Col: task.Col}}))
		if err != nil {
...
func (c *connector) Setup(ctx context.Context, t iface.Transport) error {
	...
	connectorCapabilities := iface.ConnectorCapabilities{
		...
		Sink:           capabilities.GetSink(),
		IntegrityCheck: capabilities.GetSource(),
		Resumability:   capabilities.GetResumable(),
	}
...
func adiomNamespaces(namespaces []string) []*adiomv1.Namespace {
	var res []*adiomv1.Namespace
	for _, ns := range namespaces {
		if db, col, ok := strings.Cut(ns, "."); ok {
			res = append(res, &adiomv1.Namespace{Db: db, Col: col})
		}
	}
	return res
}
...
func (c *connector) StartReadToChannel(flowId iface.FlowID, options iface.ConnectorOptions, readPlan iface.ConnectorReadPlan, dataChannelID iface.DataChannelID) error {
	...
	go func() {
		...
		c.progressTracker.ResetNsProgressEstimatedDocCounts(func(ctx context.Context, ns iface.Namespace) (int64, error) {
			res, err := c.impl.GetNamespaceMetadata(ctx, connect.NewRequest(&adiomv1.GetNamespaceMetadataRequest{Namespace: &adiomv1.Namespace{Db: ns.Db, Col: ns.Col}}))
			if err != nil {
				return 0, err
			}
	...
	go func() {
		...
		wg.Add(c.numParallelCopiers)

		for i := 0; i < c.numParallelCopiers; i++ {
			go func() {
				defer wg.Done()
	...
	go func() {
		...
		slog.Debug(fmt.Sprintf("Connector %s change stream start@ %v", c.id, c.resumeToken))
		...
		for res.Receive() {
			...
			for _, d := range msg.Updates {
				...
				dataChannel <- iface.DataMessage{
					...
					Loc: iface.Location{
						Database:   msg.GetNamespace().GetDb(),
						Collection: msg.GetNamespace().GetCol(),
					},
					Id:     &id,
					IdType: byte(d.GetId().GetType()),
					SeqNum: lsn,
				}
			}
		}
...
func (c *connector) StartWriteFromChannel(flowId iface.FlowID, dataChannelID iface.DataChannelID) error {
	...
	flowParallelWriter := NewParallelWriter(c.flowCtx, c, c.numParallelWriters, c.maxWriterBatchSize)
...
func NewLocalConnector(desc string, impl adiomv1connect.ConnectorServiceHandler, numParallelCopiers int, numParallelWriters int, maxWriterBatchSize int) iface.Connector {
	...
	slog.Info(srv.URL())
	testconnClient := adiomv1connect.NewConnectorServiceClient(srv.Client(), srv.URL())
	return NewConnector(desc, testconnClient, numParallelCopiers, numParallelWriters, maxWriterBatchSize)
}

func NewConnector(desc string, impl adiomv1connect.ConnectorServiceClient, numParallelCopiers int, numParallelWriters int, maxWriterBatchSize int) iface.Connector {
	return &connector{
		desc:               desc,
		impl:               impl,
		numParallelCopiers: numParallelCopiers,
		numParallelWriters: numParallelWriters,
		maxWriterBatchSize: maxWriterBatchSize,
	}
}
...
func (c *connector) ProcessDataMessages(dataMsgs []iface.DataMessage) error {
	...
	if len(msgs) > 0 {
		_, err := c.impl.WriteUpdates(c.flowCtx, connect.NewRequest(&adiomv1.WriteUpdatesRequest{
			Namespace: &adiomv1.Namespace{Db: dataMsgs[0].Loc.Database, Col: dataMsgs[0].Loc.Collection},
			Updates:   msgs,
			Type:      adiomv1.DataType_DATA_TYPE_MONGO_BSON, // TODO
		}))
		if err != nil {
