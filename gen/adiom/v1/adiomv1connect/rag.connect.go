...
const (
	...
	// ChunkingServiceGetChunkProcedure is the fully-qualified name of the ChunkingService's GetChunk
	// RPC.
	ChunkingServiceGetChunkProcedure = "/adiom.v1.ChunkingService/GetChunk"
...
var (
	chunkingServiceServiceDescriptor                      = v1.File_adiom_v1_rag_proto.Services().ByName("ChunkingService")
	...
	chunkingServiceGetChunkMethodDescriptor               = chunkingServiceServiceDescriptor.Methods().ByName("GetChunk")
	embeddingServiceServiceDescriptor                     = v1.File_adiom_v1_rag_proto.Services().ByName("EmbeddingService")
...
func NewChunkingServiceClient(httpClient connect.HTTPClient, baseURL string, opts ...connect.ClientOption) ChunkingServiceClient {
	...
	return &chunkingServiceClient{
		...
		getChunk: connect.NewClient[v1.GetChunkRequest, v1.GetChunkResponse](
			httpClient,
			baseURL+ChunkingServiceGetChunkProcedure,
			connect.WithSchema(chunkingServiceGetChunkMethodDescriptor),
			connect.WithClientOptions(opts...),
		),
	}
}
...
// GetChunk calls adiom.v1.ChunkingService.GetChunk.
func (c *chunkingServiceClient) GetChunk(ctx context.Context, req *connect.Request[v1.GetChunkRequest]) (*connect.Response[v1.GetChunkResponse], error) {
...
func NewChunkingServiceHandler(svc ChunkingServiceHandler, opts ...connect.HandlerOption) (string, http.Handler) {
	...
	chunkingServiceGetChunkHandler := connect.NewUnaryHandler(
		ChunkingServiceGetChunkProcedure,
		svc.GetChunk,
		connect.WithSchema(chunkingServiceGetChunkMethodDescriptor),
		connect.WithHandlerOptions(opts...),
	)
	return "/adiom.v1.ChunkingService/", http.HandlerFunc(func(w http.ResponseWriter, r *http.Request) {
		...
		case ChunkingServiceGetChunkProcedure:
			chunkingServiceGetChunkHandler.ServeHTTP(w, r)
		default:
			http.NotFound(w, r)
		}
	})
...
func (UnimplementedChunkingServiceHandler) GetChunk(context.Context, *connect.Request[v1.GetChunkRequest]) (*connect.Response[v1.GetChunkResponse], error) {
	return nil, connect.NewError(connect.CodeUnimplemented, errors.New("adiom.v1.ChunkingService.GetChunk is not implemented"))
}
